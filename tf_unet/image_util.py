...
with adjustments by samarinm (13.12.20)
'''
...
class SimpleDataProvider_withIndex(SimpleDataProvider):
    """
    samarinm added: As base class "SimpleDataProvider" with _next_data() also printing index of sample idx.
    """
    def __init__(self, *args, **kwargs):
        super(SimpleDataProvider_withIndex, self).__init__(*args, **kwargs)


    def _next_data(self):
        idx = np.random.choice(self.file_count)
        print("\n------- SimpleDataProvider: Index of image sample =", idx)
        return self.data[idx], self.label[idx]
